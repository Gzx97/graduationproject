...
var url;
	...
	function openShangpinAddDialog(){
		$("#dlg").dialog("open").dialog("setTitle","添加视频素材");
		url="../addShangpin?shangpinType=1";
	}
	...
	function openShangpinModifyDialog(){
		...
		$("#dlg").dialog("open").dialog("setTitle","编辑视频素材");
		$("#fm").form("load",row);
	...
	function formaTupian(shujuImgName, row) {
		if(shujuImgName){
			return '<a target="_blank" style="color:red;" href="../downFile?filename=' + shujuImgName + '">预览' + '</a>'; 
		}else {
			return "未上传";
		}
	}
	...
	function daoruShangpins(){
		$("#daoru").dialog("open").dialog("setTitle","导入视频素材");
	...
	function shangchuanShangpin(){
		...
		var row=selectedRows[0];
		$("#shangchuan").dialog("open").dialog("setTitle","上传视频素材");
...
<script src="../shipin/js/jquery.poptrox.min.js"></script>
<script>
	
	
	function shangchuanSpshipin(){
		var selectedRows=$("#dg").datagrid('getSelections');
		if(selectedRows.length!=1){
			$.messager.alert("系统提示","请选择一条要编辑的数据！");
			return;
		...
		$("#shangchuan").dialog("open").dialog("setTitle","上传视频素材");
		$("#shchfm").form("load",row);
		shchurl="../shangchuanSpshipin?shangpinId="+row.shangpinId;
	}
	$(function() {				
		$('#gallery').poptrox({
	...
	function shipinChakan(shujuImg, row) {
		if(shujuImg){
			return '<a target="_blank" data-poptrox="youtube,600x380" href="../' + shujuImg + '">查看' + '</a>'; 
		}else {
			return "未上传";
		}
	}
...
<!--startprint-->
	<table id="dg" title="视频素材" class="easyui-datagrid" fitColumns="true"
	 pagination="true" url="../getShangpins?shangpinType=1" fit="true" toolbar="#tb">
		<thead>
			<tr>
				<th field="cb" checkbox="true"></th>
				<th field="shangpinId" width="20">编号</th>
				<th field="shangpinName" width="40">名称</th>
				<th field="shangpinMark1" width="40">作者</th>
				<th field="shangpinMark" width="80">详情</th>
				<th field="sptypeId" width="20" hidden="true">类型ID</th>
				<th field="sptypeName" width="40">类型</th>
				<th field="roleName" width="20" formatter="formatXiazai">视频</th>
				<th field="shangpinImg" width="20" formatter="formatChakan">预览</th>
				<th field="shangpinDate" width="80" formatter="datetostr">时间</th>
			</tr>
		</thead>
	</table>
	...
	<div id="tb">
		<div>
			...
			<a href="javascript:shangchuanSpshipin()" class="easyui-linkbutton" iconCls="icon-add" plain="true">上传视频</a>
			<a href="javascript:shangchuanShangpin()" class="easyui-linkbutton" iconCls="icon-add" plain="true">上传视频图片</a>
		</div>
		<div>
